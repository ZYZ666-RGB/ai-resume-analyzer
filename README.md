...
- pytest 覆盖核心规则，GitHub Actions 自动执行后端测试与前端构建。
...
sequenceDiagram
    ...
    API->>Cache: GET resume:parse:{resume_hash}
    ...
    API->>Cache: GET resume:match:{resume_hash}:{jd_hash}
...
| `RETURN_CLEANED_TEXT` | `true` | 是否在解析响应返回清洗文本；公网环境建议关闭 |
...
```json
{
  ...
  "data": {
    ...
    "pageCount": 2,
    "resume": {"basicInfo": {"name": "某候选人", "phone": "已脱敏展示", "email": "已脱敏展示"}},
    "match": {
      "overallScore": 82,
      "skillScore": 90,
      "experienceScore": 80,
...
未配置 AI Key 或 AI 输出未通过结构校验时，`aiScore` 使用规则综合分（技能 45%、经验 25%、项目 20%、学历 10%），因此接口仍可在纯规则模式下给出确定性结果。
...
jd_hash     = SHA256(jobTitle + jobDescription)

resume:parse:{resume_hash}
resume:match:{resume_hash}:{jd_hash}
```
...
Redis 操作全部可降级：超时、鉴权或连接失败只记录不含密码/简历正文的 warning 并继续分析。缓存可能包含结构化简历信息，生产 Redis 必须使用私网、TLS/鉴权、最小权限和受控 TTL；如合规要求禁止缓存个人信息，请设置 `REDIS_ENABLED=false`。
...
- 使用固定系统约束和“简历原文/岗位描述”等清晰标签，把输入限定为抽取依据。当前只是基础防护，生产仍需更强的数据分隔、注入样本测试与输出审计。
...
| 502 | AI 服务拒绝、返回不可恢复的非法结果 |
| 504 | AI 服务请求超时 |

全局异常处理器保持统一响应格式。日志可记录路径、耗时、页数、缓存命中、AI 耗时、Redis 降级和错误类型，但不得记录完整简历、完整手机号/邮箱、API Key、Redis 密码或 Python 堆栈到客户端。
...
Pages 是静态前端备选方案。构建时同样注入公网 `VITE_API_BASE_URL`，Vite `base` 需与仓库子路径匹配（例如 `/ai-resume-analyzer/`）。推荐使用 GitHub 官方 `configure-pages`、`upload-pages-artifact`、`deploy-pages` Actions 发布 `frontend/dist`。
...
CI 在 push/PR 时分别运行 pytest 和 `npm ci && npm run build`，不会调用真实 AI 或 Redis。建议上线前另做一轮仅使用授权脱敏样本的集成测试。
...
- `RETURN_CLEANED_TEXT` 在公网环境建议设为 `false`，以减少个人信息在网络和前端暴露。
...
- 为 Prompt、模型与别名字典增加版本号、离线评测集和可解释的证据引用。
...
- 加入端到端测试、契约测试、依赖扫描、镜像扫描和正式 CD 审批。
...
- [ ] `GET /api/health` 返回统一成功结构，Swagger 可访问。
- [ ] 合法多页文本 PDF 可解析；非 PDF、损坏、空文本、超 10 MB 均返回正确状态码。
- [ ] 电话/邮箱正则优先，AI 非法 JSON 可控，所有模型分值都限制在 0～100。
- [ ] 同一 PDF/JD 二次调用显示缓存命中；关闭 Redis 后主流程仍运行且无内部连接信息泄露。
- [ ] 前端能上传/移除 PDF、填写示例 JD、防重复提交、显示阶段和完整可视化结果。
- [ ] 桌面端与移动端布局可用，错误状态清楚，不只展示原始 JSON。
- [ ] `cd backend && pytest` 通过，测试未真实调用通义千问或 Redis。
- [ ] `cd frontend && npm install && npm run build` 通过，生产产物不含 localhost 或密钥。
- [ ] `docker compose config` 与 `docker compose build` 通过，容器监听 `0.0.0.0:8000`。
