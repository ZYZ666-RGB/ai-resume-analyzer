...
### 8.2 官方 Pages Actions 工作流示例

可另建 `.github/workflows/pages.yml`。以下配置使用官方 `configure-pages`、`upload-pages-artifact`、`deploy-pages` Actions；它不会在本仓库中自动创建或执行部署：
...
在 Settings → Secrets and variables → Actions → Variables 添加公开变量 `VITE_API_BASE_URL=https://<function-domain>`。API 地址本身不是秘密；真正的 Key 永远不能进入 Pages 构建。
...
- [ ] 本地 `pytest` 和 `npm run build` 成功，AI/Redis 在单元测试中被 Mock。
